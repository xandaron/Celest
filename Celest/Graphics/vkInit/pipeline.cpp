...
inline void vkInit::buildPipeline(const vk::Device& device, const PipelineBuildInfo& buildInfo, Pipeline& pipeline) {
	createLayout(device, buildInfo.descriptorSetLayouts, pipeline.layout);
	createRenderPass(device, buildInfo.overwrite, buildInfo.depthAttachmentInfo, buildInfo.colourAttachmentInfo, pipeline.renderPass);
	createShaderStages(device, buildInfo.shaderStages, pipeline.shaderStages);
	/*
	...
	*/
	vk::GraphicsPipelineCreateInfo pipelineInfo{
		...
		static_cast<uint32_t>(pipeline.shaderStages.size()),
		pipeline.shaderStages.data(),
	...
	createPipeline(device, pipelineInfo, pipeline.pipeline);
}

inline vk::PipelineShaderStageCreateInfo vkInit::createShaderInfo(const vk::Device& device, const std::string& filename, const vk::ShaderStageFlagBits& stage) {
	/*
...
inline void vkInit::createShaderStages(
	...
	for (int i = 0; i < shaderStages.size(); i++) {
		...
		stages[i] = createShaderInfo(device, shaderStages[i].shaderPath, shaderStages[i].flag);
	}
}
...
inline void vkInit::createLayout(const vk::Device& device, const std::vector<vk::DescriptorSetLayout>& descriptorSetLayouts, vk::PipelineLayout& layout) {
	Debug::Logger::log(Debug::MESSAGE, "Creating pipeline layout!");
	try {
		layout = device.createPipelineLayout(createLayoutInfo(descriptorSetLayouts));
	}
...
inline std::vector<vk::AttachmentDescription> vkInit::createAttachmentDescriptions(
...
inline std::vector<vk::AttachmentReference> vkInit::createAttachmentReferences(const AttachmentInfo& depthAttachment, const uint32_t& colourAttachmentIndex) {
	/*
	AttachmentReference(
...
inline void vkInit::createRenderPass(
...
inline void vkInit::createPipeline(const vk::Device& device, const vk::GraphicsPipelineCreateInfo& pipelineInfo, vk::Pipeline& pipeline) {
